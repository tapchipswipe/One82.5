...
const getMerchantRows = (transactions: Transaction[], profiles: BuyRateProfile[]): MerchantProfitRow[] => {
  ...
  return Array.from(grouped.entries())
    .map(([name, records]) => {
      ...
      // Performance optimization: compute first and second half sums in a single loop without slice allocations
      let firstHalf = 0;
      let secondHalf = 0;
...
const Profitability: React.FC = () => {
  ...
  const totals = useMemo(() => {
    ...
    // Performance optimization: Single pass iteration instead of 5 consecutive reduce calls
