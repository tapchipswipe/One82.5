...
const getMerchantRows = (transactions: Transaction[], profiles: BuyRateProfile[]): MerchantProfitRow[] => {
  ...
  return Array.from(grouped.entries())
    .map(([name, records]) => {
      // ⚡ Bolt: Single pass loop replacing reduce() for volume
      let volume = 0;
      for (let i = 0; i < records.length; i++) {
        volume += records[i].amount;
      }
      ...
      // ⚡ Bolt: Replacing multiple slice().reduce() passes with single loop to prevent intermediate allocations
      let firstHalf = 0;
      let secondHalf = 0;
...
const Profitability: React.FC = () => {
  ...
  const totals = useMemo(() => {
    // ⚡ Bolt: Replaced 5 sequential map/reduce passes with a single loop
    // Expected impact: O(5N) -> O(N) traversal, saves CPU cycles on large tables
    let totalVolume = 0;
    let totalMargin = 0;
