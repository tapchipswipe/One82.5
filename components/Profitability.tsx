...
const getMerchantRows = (transactions: Transaction[], profiles: BuyRateProfile[]): MerchantProfitRow[] => {
  ...
  return Array.from(grouped.entries())
    .map(([name, records]) => {
      ...
      // ⚡ Bolt Optimization: Replaced slice().reduce() with index-bound for loops to eliminate intermediate array allocations.
      let firstHalf = 0;
...
const Profitability: React.FC = () => {
  ...
  const totals = useMemo(() => {
    // ⚡ Bolt Optimization: Replaced multiple reduce() calls with a single-pass for loop to eliminate redundant array traversals.
    let totalVolume = 0;
    let totalMargin = 0;
