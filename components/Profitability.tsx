...
const getMerchantRows = (transactions: Transaction[], profiles: BuyRateProfile[]): MerchantProfitRow[] => {
  ...
  return Array.from(grouped.entries())
    .map(([name, records]) => {
      ...
      // Replaced two .slice().reduce() calls with a single O(N) pass to calculate trend halves.
      // This prevents allocating two intermediate sub-arrays per merchant, saving memory
      // and reducing iteration overhead when processing large transaction portfolios.
      let firstHalf = 0;
      let secondHalf = 0;
