...
const getMerchantRows = (transactions: Transaction[], profiles: BuyRateProfile[]): MerchantProfitRow[] => {
  ...
  return Array.from(grouped.entries())
    .map(([name, records]) => {
      // ⚡ Bolt: Replace multiple array sweeps with single pass logic
      let volume = 0;
      ...
      // ⚡ Bolt: Single pass accumulation instead of slice() + reduce() for better performance
      let firstHalf = 0;
      let secondHalf = 0;
...
const Profitability: React.FC = () => {
  ...
  const totals = useMemo(() => {
    // ⚡ Bolt: Single pass accumulation instead of multiple reduce calls for better performance
    let totalVolume = 0;
    let totalMargin = 0;
