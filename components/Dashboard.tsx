...
const Dashboard: React.FC<DashboardProps> = ({ businessType, onNavigate }) => {
  ...
  const fetchInsights = useCallback(async (force: boolean = false) => {
    ...
    // ⚡ Bolt: Consolidated map/filter/sort into a single O(N) loop to find latest transaction timestamp
    // Impact: Reduces O(N log N) sorting overhead and prevents redundant array allocations for large datasets
