...
const Dashboard: React.FC<DashboardProps> = ({ businessType, onNavigate }) => {
  ...
  const fetchInsights = useCallback(async (force: boolean = false) => {
    ...
    // ⚡ Bolt: Consolidated mapping, filtering, and O(N log N) sorting into a single O(N) loop to track the maximum timestamp. Impact: Reduces redundant traversals and eliminates sorting overhead.
    let newestTxTimestamp: number | null = -Infinity;
    for (let i = 0; i < transactions.length; i++) {
      const ts = new Date(transactions[i].date).getTime();
      if (Number.isFinite(ts) && ts > newestTxTimestamp) {
