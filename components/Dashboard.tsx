...
const Dashboard: React.FC<DashboardProps> = ({ businessType, onNavigate }) => {
  ...
  const fetchInsights = useCallback(async (force: boolean = false) => {
    ...
    // ⚡ Bolt: Consolidated chained O(N log N) array methods (.map.filter.sort) into a single O(N) loop
    // Impact: Reduces O(N log N) + O(N) operations to a single O(N) iteration, saving memory and CPU.
    ...
    if (newestTxTimestamp === -Infinity) {
      newestTxTimestamp = null;
    }
