...
const Dashboard: React.FC<DashboardProps> = ({ businessType, onNavigate }) => {
  ...
  const fetchInsights = useCallback(async (force: boolean = false) => {
    ...
    // ⚡ Bolt: Consolidated chained map/filter/sort into a single O(N) loop to find the latest timestamp. Impact: Reduces O(N log N) + multiple iterations down to a single O(N) pass, reducing UI freezing on large datasets.
