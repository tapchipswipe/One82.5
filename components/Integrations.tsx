...
const IntegrationCard = ({ integration, onSave }: {
    ...
    const setupGuide = getIntegrationSetupGuide(integration.id);

    const handleSave = () => {
        const normalizedKey = apiKeyValue.trim();
        const validationError = validateIntegrationCredential(integration.id, normalizedKey);
        if (validationError) {
            setCredentialError(validationError);
            return;
        }
        saveIntegrationKey(integration.id, normalizedKey);
        setCredentialError(null);
        setApiKeyValue(normalizedKey);
        setExpanded(false);
        onSave(integration.id, normalizedKey.length > 0);
    };
        ...
        <div className={`bg-white dark:bg-gray-800 rounded-xl border transition-all ${connected
            ...
            {expanded && (
                <div className="px-5 pb-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                    <div className="rounded-lg border border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/70 dark:bg-indigo-900/10 p-3 mb-4">
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                            <p className="text-[11px] font-semibold text-indigo-900 dark:text-indigo-200">
                                Setup path for {integration.name}
                                {setupGuide.recommendedStarter && ' (recommended starter)'}
                            </p>
                            <a
                                href={setupGuide.portalUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-[11px] font-semibold text-indigo-700 dark:text-indigo-300 hover:underline inline-flex items-center gap-1"
                            >
                                Open {setupGuide.portalLabel}
                                <ExternalLink className="w-3 h-3" />
                            </a>
                        </div>
                        <ol className="mt-2 space-y-1 text-[11px] text-indigo-900 dark:text-indigo-200 list-decimal pl-4">
                            {setupGuide.setupSteps.map((step) => (
                                <li key={step}>{step}</li>
                            ))}
                        </ol>
                    </div>
                    <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2">
                        {setupGuide.credentialLabel}
                    </label>
                    <div className="flex gap-2 flex-wrap">
                        <div className="relative flex-1 min-w-[240px]">
                            <input
                                ...
                                placeholder={setupGuide.keyPlaceholder}
                                className={`w-full px-3 py-2 pr-10 border rounded-lg text-sm bg-gray-50 dark:bg-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none font-mono ${credentialError ? 'border-red-300 dark:border-red-700' : 'border-gray-200 dark:border-gray-700'}`}
                            />
                            <button
                                type="button"
                    ...
                    {credentialError && (
                        <p className="text-[11px] text-red-600 mt-2">{credentialError}</p>
                    )}
