...
const buildPortfolioFromTransactions = (transactions: Transaction[]): PortfolioMerchant[] => {
    ...
    return Array.from(grouped.entries()).map(([name, records], index) => {
        ...
        // Optimization: Replaced .slice().reduce() with single-pass for-loop
        // to eliminate intermediate array allocations and reduce CPU cycles.
        let firstHalf = 0;
        let secondHalf = 0;
        ...
        const volumeHistory = Array.from({ length: 6 }, (_, offset) => {
            ...
            // Optimization: Avoided .slice().reduce() inside loop to prevent
            // O(N) array copying per slice iteration.
            let sum = 0;
