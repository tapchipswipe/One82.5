...
const ISODashboard: React.FC<ISODashboardProps> = ({ onNavigate }) => {
    ...
    const uniqueIndustriesSet = new Set<string>();
    ...
    for (let i = 0; i < merchants.length; i++) {
        ...
        estMonthlyResidual += m.monthlyVolume * ((m.bps || 0) / 10000);
        avgBpsSum += (m.bps || 0);
        if (m.monthlyVolume > topMerchantVolume) topMerchantVolume = m.monthlyVolume;
        uniqueIndustriesSet.add(m.businessType);
    ...
    const avgBps = merchants.length > 0 ? avgBpsSum / merchants.length : 0;
    const atRiskRate = merchants.length > 0 ? (atRiskCount / merchants.length) * 100 : 0;
    const uniqueIndustries = uniqueIndustriesSet.size;
