...
const buildPortfolioFromTransactions = (transactions: Transaction[]): PortfolioMerchant[] => {
    ...
    return Array.from(grouped.entries()).map(([name, records], index) => {
        ...
        let firstHalf = 0;
        let secondHalf = 0;
        // Optimization: Single-pass loop to calculate halves instead of .slice().reduce()
        // Prevents intermediate array allocations to reduce memory and CPU overhead.
        ...
        const volumeHistory = Array.from({ length: 6 }, (_, offset) => {
            ...
            // Optimization: Iterate directly to avoid .slice().reduce() allocations.
            for (let i = start; i < actualEnd && i < records.length; i++) {
