...
const ISODashboard: React.FC<ISODashboardProps> = ({ onNavigate }) => {
    ...
    // Optimization: Calculate all metrics in a single O(N) pass to prevent performance degradation on large portfolios
    let atRiskCount = 0;
    ...
    for (let i = 0; i < merchants.length; i++) {
        ...
        sumBps += (m.bps || 0);
        if ((m.monthlyVolume || 0) > topMerchantVolume) topMerchantVolume = m.monthlyVolume || 0;
        industries.add(m.businessType);
