...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Hardcoded Admin Login - Updated to 'admin' / 'admin'
      if (email === 'admin' && password === 'admin') {
        const adminUser: User = {
          id: 'admin_' + Date.now(),
          email: 'admin',
          name: 'Administrator',
          businessType: 'Retail', // Admin default view
      ...
      if (!user || user.email !== email) {
        // Create new standard user simulation
        user = {
            id: 'u_' + Date.now(),
            email,
            name: email.split('@')[0] || 'User',
            businessType: 'Retail', // Default, will be updated in Onboarding
