...
const createCookieSession = (user: User): AuthSession => {
  ...
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random number generator not available. Cannot generate session ID securely.');
  }
  return {
    sessionId: `backend_session_${Date.now()}_${(crypto as any).randomUUID().slice(0, 8)}`,
    userId: user.id,
    tenantId,
    role: user.role,
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random number generator not available. Cannot generate session token securely.');
  }
