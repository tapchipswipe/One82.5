...
const createCookieSession = (user: User): AuthSession => {
  ...
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is unavailable. Cannot generate session ID securely.');
  }

  return {
    sessionId: `backend_session_${Date.now()}_${(crypto as any).randomUUID().replace(/-/g, '').slice(0, 8)}`,
    userId: user.id,
    tenantId,
    role: user.role,
