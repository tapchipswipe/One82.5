...
const createCookieSession = (user: User): AuthSession => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is unavailable. Cannot generate a secure session ID.');
  }
  ...
  return {
    sessionId: `backend_session_${Date.now()}_${crypto.randomUUID().slice(0, 8)}`,
    userId: user.id,
    tenantId,
    role: user.role,
...
const createSessionToken = (): string => {
  ...
  throw new Error('Secure randomness is unavailable. Cannot generate a secure session token.');
};
