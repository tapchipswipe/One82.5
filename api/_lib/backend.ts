...
const createCookieSession = (user: User): AuthSession => {
  ...
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation is unavailable. Failing securely to prevent predictable session generation.');
  }
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation is unavailable. Failing securely to prevent predictable session tokens.');
  }
