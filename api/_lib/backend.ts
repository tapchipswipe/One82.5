...
const createCookieSession = (user: User): AuthSession => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is unavailable. Cannot generate session ID.');
  }
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is unavailable. Cannot generate session token.');
  }
