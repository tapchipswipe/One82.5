...
const createCookieSession = (user: User): AuthSession => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation is not available. Cannot generate secure session ID.');
  }
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation is not available. Cannot generate secure session token.');
  }
