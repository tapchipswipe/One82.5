...
const createCookieSession = (user: User): AuthSession => {
  ...
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is not available. Cannot generate session ID.');
  }
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure randomness is not available. Cannot generate session token.');
  }
