...
// 🛡️ Sentinel: Generate secure random strings instead of Math.random() for session identifiers
...
const createCookieSession = (user: User): AuthSession => {
  ...
  return {
    sessionId: `backend_session_${Date.now()}_${generateSecureString(8)}`,
    userId: user.id,
    tenantId,
    role: user.role,
...
const createSessionToken = (): string => {
  ...
  return `st_${Date.now().toString(36)}_${generateSecureString(16)}`;
};
