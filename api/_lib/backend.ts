...
const createCookieSession = (user: User): AuthSession => {
  ...
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation not available. Aborting session creation.');
  }

  return {
    // SECURITY: Use secure randomness for session identifiers, no insecure fallbacks
...
const createSessionToken = (): string => {
  if (typeof crypto === 'undefined' || !('randomUUID' in crypto)) {
    throw new Error('Secure random generation not available. Aborting token creation.');
  }

  // SECURITY: Use secure randomness for session tokens, no insecure fallbacks
