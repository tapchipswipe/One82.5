...
const buildPortfolioFromTransactions = (transactions: Transaction[]): PortfolioMerchant[] => {
  ...
  return Array.from(grouped.entries()).map(([name, records], index) => {
    ...
    // Replaced .slice().reduce() with a single-pass for-loop to prevent allocating intermediate arrays.
    // Impact: Reduces garbage collection overhead and improves execution speed for large transaction sets.
    ...
    const volumeHistory = Array.from({ length: 6 }, (_, offset) => {
        // ⚡ Bolt Performance Optimization
        // Replaced .slice().reduce() with a for-loop bounded by the segment indices to eliminate O(N) array copying.
        const start = Math.floor((offset * records.length) / 6);
        const end = Math.max(start + 1, Math.floor(((offset + 1) * records.length) / 6));
        let segmentSum = 0;
        for (let i = start; i < end && i < records.length; i++) {
            segmentSum += records[i].amount;
        }
        return Math.round(segmentSum);
    });
